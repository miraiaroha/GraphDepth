...
from dataset.nust_dataloader import NustFromTxt
sys.path.append('./model')
from model.multiscale_model import MultiScaleModel
from model.deeplab_model import DeepLabResNetModel
from model.dorn_model import DornModel
from model.attention_model import AttentionModel
from model.pixel_attention_model import PixelAttentionModel
from model.context_attention_model import ContextAttentionModel
from model.ACAN import ACAN
from model.ACAN1 import ACAN1
from model.ACAN2 import ACAN2
from model.ACAN3 import ACAN3
from model.ACAN4 import ACAN4
...
def main():
    mode = 'train'
    test_index = 45
    dataset_name_list = ['nyu', 'kitti', 'nust']
    model_name_list = ['attention', 'deeplab', 'dorn',
                       'multiscale', 'pixelattention', 'contextattention',
                       'ACAN', 'ACAN1', 'ACAN2', 'ACAN3', 'ACAN4']
    dataset_dict = {'nyu': NyuFromTxt,
                    'kitti': KittiFromTxt, 'nust': NustFromTxt}
    model_dict = {'attention': AttentionModel, 'deeplab': DeepLabResNetModel,
                  'dorn': DornModel, 'multscale': MultiScaleModel,
                  'pixelattention': PixelAttentionModel, 'contextattention': ContextAttentionModel,
                  'ACAN': ACAN, 'ACAN1': ACAN1, 'ACAN2': ACAN2, 'ACAN3': ACAN3, 'ACAN4': ACAN4}
...
def train(params, myModel, criterion):
    ...
    dummy_input = Variable(torch.rand(
        params.batch_size, 3, params.crop_height, params.crop_width)).cuda()
    modelsize(myModel, dummy_input)
    del dummy_input
    # myModel.load_state_dict(torch.load(
    #     '../../checkpoint/contextattention_nyu/best.pkl'))
    myModel.cuda()
    # print(repr(myModel))
    # filter manully
    ...
    for epoch in range(params.num_epochs):
        ...
        for step, (images, depths) in enumerate(trainloader):
            ...
            lr = adjust_learning_rate(
                optimizer, global_step + 1, num_total_steps, epoch + 1, params.num_epochs, params.learning_rate_mode)
            ...
            # compute loss
            if params.model_name in ['contextattention', 'ACAN', 'ACAN1', 'ACAN2', 'ACAN3', 'ACAN4']:
                image_loss, auxi_loss, total_loss = criterion(
                    predict, depths, myModel.get_sim_map(), epoch + 1)
            else:
            ...
            # print loss
            print_interval = {'nyu': 100, 'kitti': 200, 'nust': 200}
            if (step + 1) % (steps_per_epoch // print_interval[params.dataset_name]) == 0:
                print_str = 'Epoch [{:>3}/{:>3}] | Step [{:>3}/{:>3}] | fps {:4.2f} | Image Loss: {:7.3f} | Auxi Loss: {:7.3f} | Total Loss: {:7.3f} | Time elapsed {:.2f}h | Time_left {:.2f}h'. \
                    format(epoch + 1, params.num_epochs, step + 1, steps_per_epoch, fps,
                           image_loss, auxi_loss, total_loss, time_sofar, time_left)
                print(print_str)
        ...
        if (epoch + 1) % params.interval_test == 0:
            ...
            test_total_time = 0
            with torch.no_grad():
                measure_list = ['a1', 'a2', 'a3', 'rmse',
                                'rmse_log', 'log10', 'abs_rel', 'sq_rel']
                ...
                for index, (images, depths) in enumerate(validloader):
                    ...
                    for i in range(len(measure_list)):
                        measures[measure_list[i]
                                 ] += new[measure_list[i]].item()
                    # display images
                    if index == 70:
                        display_figure(params, myModel, criterion, writer,
                                       images, depths, predict, epoch)

                measures_str = {key: '{:.5f}'.format(
                    value / num_validation_samples) for key, value in measures.items()}
                #gpu_profile(frame=sys._getframe(), event='line', arg=None)
                fps = num_validation_samples / test_total_time
                print_str = 'The {}th epoch, fps {:4.2f} | {}'.format(
                    epoch + 1, fps, measures_str)
                print(print_str)
                write_to_log(LOG_DIR, print_str)
                for i in range(len(measure_list)):
                    writer.add_scalar(
                        measure_list[i], measures[measure_list[i]] / num_validation_samples, epoch + 1)
                # saving the model
...
def display_images(params, myModel, criterion, writer, images, depths, predict, epoch):
    # display image and depth
    b, h, w = depths.shape[0], params.crop_height, params.crop_width
    rgb = F.interpolate(images, size=(h // 2, w // 2),
                        mode='bilinear', align_corners=True)
    gt = F.interpolate(depths, size=(h // 2, w // 2), mode='nearest')
    gt = torch.cat((gt, gt, gt), 1)
    if params.model_name in ['attention', 'pixelattention', 'contextattention', 'ACAN']:
        sim_map = myModel.get_sim_map()
        points = [i * 100 for i in range(4)]
        sim_pixel = sim_map[:, points]
        sim_pixel = (torch.max(sim_pixel) - sim_pixel) / \
            (torch.max(sim_pixel) - torch.min(sim_pixel))
        sim_pixel = sim_pixel.reshape((b, len(points), h // 8, w // 8))
        sim_pixel = F.interpolate(
            sim_pixel, size=(h // 2, w // 2), mode='bilinear', align_corners=True)
        sim_pixel = sim_pixel.reshape((-1, 1, h // 2, w // 2))
        grid_ = torchvision.utils.make_grid(sim_pixel, nrow=depths.shape[0])
        writer.add_image('sim_map', grid_, epoch + 1)
    if params.model_name in ['multiscale', 'attention']:
        pred = []
        for i in range(len(predict)):
            scale = F.interpolate(myModel.inference(
                predict[i]), size=(h // 2, w // 2), mode='nearest')
            scale = torch.cat((scale, scale, scale), 1)
            pred = pred + [scale]
        pred = torch.cat(pred, 0)
        fuse = torch.cat((rgb, gt, pred), 0)
    else:
        pred = F.interpolate(myModel.inference(predict),
                             size=(h // 2, w // 2), mode='nearest')
        pred = torch.cat((pred, pred, pred), 1)
        fuse = torch.cat((rgb, gt, pred), 0)
    grid = torchvision.utils.make_grid(fuse, nrow=depths.shape[0])
    writer.add_image('pictures', grid, epoch + 1)
    return
...
def display_figure(params, myModel, criterion, writer, images, depths, predict, epoch):
    ...
    if params.model_name in ['pixelattention', 'contextattention', 'ACAN', 'ACAN1', 'ACAN2', 'ACAN3', 'ACAN4']:
    ...
    if params.model_name in ['contextattention', 'ACAN', 'deeplab', 'ACAN2', 'ACAN4']:
...
def test(params, myModel, test_index):
    ...
    LOG_DIR = params.save_path + '{0}_{1}_measures.txt'.format(
        params.model_name, params.dataset_name)
    myModel.load_state_dict(torch.load(
    ...
    with torch.no_grad():
        ...
        for index, (images, depths) in enumerate(validloader):
            ...
            # np.save(
            #     SAVE_DIR + '{:04}_context.npy'.format(index), image_context)
            for i in range(len(measure_list)):
