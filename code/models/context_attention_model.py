...
class ContextAttentionModel(nn.Module):
    ...
    def soft_ordinal_regression(self, pred_prob, Min, Max, num_classes):
        # if pred_prob.is_cuda:
        #     Min, Mac, NC = Min.cuda(), Max.cuda(). num_classes.cuda()
        # nc = torch.arange(NC.item)
        # if pred_prob.is_cuda:
        #     nc = nc.cuda()
        # weight = nc *
    ...
    class LossFunc(nn.Module):
        def __init__(self, min_depth, max_depth, num_classes, classifierType, scale=1, alpha=1, ignore_index=None):
